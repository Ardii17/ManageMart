...
<html lang="en">
  ...
  <body>
    <div class="flex">
      <aside
        class="flex flex-col items-center justify-between w-1/5 h-screen text-black shadow bg-emerald-100 "
      >
      ...
      <main class="relative w-4/5 h-screen bg-gray-100">
        ...
        <div class="absolute top-0 bottom-0 left-0 right-0">
          <div class="flex items-center gap-4 mx-4 my-4 mb-12">
            ...
            <p class="w-40 px-4 py-2 font-semibold text-center rounded bg-emerald-300">Edit Products</p>
          </div>
          <div class="flex justify-center w-full">
            <div class="h-auto p-4 bg-white rounded-lg shadow-md w-96">
              <form action="editproduct.php" method="POST" class="flex flex-col gap-4">
                <input type="hidden" name="id" value="<?php echo $product['id']; ?>">
                <div class="flex flex-col gap-2">
                  <label for="name" class="text-black">Name Product</label>
                  <input type="text" id="name" name="name" value="<?php echo $product['name']; ?>" required class="p-2 border-2 rounded outline-white">
                </div>
                <div class="flex flex-col gap-2">
                  <label for="category" class="text-black">Category Product</label>
                  <input type="text" id="category" name="category" value="<?php echo $product['category']; ?>" required class="p-2 border-2 rounded outline-white">
                </div>
                <div class="flex flex-col gap-2">
                  <label for="price" class="text-black">Price Product</label>
                  <input type="number" id="price" name="price" value="<?php echo $product['price']; ?>" required class="p-2 border-2 rounded outline-white">
                </div>
                <div class="flex flex-col gap-2">
                  <label for="stock" class="text-black">Stock Product</label>
                  <input type="number" id="stock" name="stock" value="<?php echo $product['stock']; ?>" required class="p-2 border-2 rounded outline-white">
                </div>
                <button type="submit" class="px-3 py-2 font-semibold text-white rounded place-content-end bg-emerald-500">Update</button>
              </form>
            </div>
          </div>
        </div>
